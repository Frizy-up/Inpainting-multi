...
model_path = '../models/Single-random/'
result_path= '../results/Single-random/'
...
for epoch in range(n_epochs):
    ...
    for start,end in zip(
        ...
        images_ori = map(lambda x: load_image_random( x, height=resultDim,width=resultDim), image_paths)
        ...
        images_crops = map(lambda x: crop_random_damage(x, x=fill_one_side, y=fill_one_side), images_ori)
        ...
        if iters % 500 == 0:
            ...
            test_images_ori = map(lambda x: load_image_random(x,height=resultDim, width=resultDim), test_image_paths)

            # Frizy
            # test_images_crop = map(lambda x: crop_random(x, x=32, y=32), test_images_ori)
            test_images_crop = map(lambda x: crop_random_damage(x, width=hiding_size, height=hiding_size, x=fill_one_side, y=fill_one_side), test_images_ori)
            ...
            if iters % 500 == 0:
                ii = 0
                for rec_val, img in zip(reconstruction_vals, test_images):
