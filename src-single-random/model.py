...
class Model():
    ...
    def channel_wise_fc_layer(self, input, name): # bottom: (7x7x512)
        ...
        with tf.variable_scope(name):
            ...
            # Frizy changed: batch_matmul-->matmul
    ...
    def batchnorm(self, bottom, is_train, epsilon=1e-8, name=None):
        ...
        normed = tf.contrib.layers.batch_norm(bottom, 0.5, updates_collections=None, epsilon=epsilon,
    ...
    def build_reconstruction_random( self, images, is_train ):
        batch_size = images.get_shape().as_list()[0]

        with tf.variable_scope('GEN'):
            # Frizy added : for 256->128
            conv1 = self.new_conv_layer(images, [4, 4, 3, 32], stride=2, name="conv1")
            bn1 = self.leaky_relu(self.batchnorm(conv1, is_train, name='bn1'))

            # 128-->64
            conv2 = self.new_conv_layer(bn1, [4, 4, 32, 32], stride=2, name="conv2")
            bn2 = self.leaky_relu(self.batchnorm(conv2, is_train, name='bn2'))

            # 64-->32
            conv3 = self.new_conv_layer(bn2, [4,4,32,64], stride=2, name="conv3")
            bn3 = self.leaky_relu(self.batchnorm(conv3, is_train, name='bn3'))

            # 32-->16
            conv4 = self.new_conv_layer(bn3, [4,4,64,128], stride=2, name="conv4")
            bn4 = self.leaky_relu(self.batchnorm(conv4, is_train, name='bn4'))

            # 16-->8
            conv5 = self.new_conv_layer(bn4, [4,4,128,256], stride=2, name="conv5")
            bn5 = self.leaky_relu(self.batchnorm(conv5, is_train, name='bn5'))


            channel_fc = self.channel_wise_fc_layer(bn5,name="channel_wise_fc")


            # 8,8,256-->8,8,256
            deconv5 = self.new_deconv_layer( channel_fc, [4,4,256,256], conv5.get_shape().as_list(), stride=1, name="deconv5")
            debn5 = tf.nn.relu(self.batchnorm(deconv5, is_train, name='debn5'))

            # 8->16
            deconv4 = self.new_deconv_layer( debn5, [4,4,128,256], conv4.get_shape().as_list(), stride=2, name="deconv4")
            debn4 = tf.nn.relu(self.batchnorm(deconv4, is_train, name='debn4'))

            # 16->32
            deconv3 = self.new_deconv_layer( debn4, [4,4,64,128], conv3.get_shape().as_list(), stride=2, name="deconv3")
            debn3 = tf.nn.relu(self.batchnorm(deconv3, is_train, name='debn3'))

            # 32->64
            deconv2 = self.new_deconv_layer( debn3, [4,4,32,64], conv2.get_shape().as_list(), stride=2, name="deconv2")
            debn2 = tf.nn.relu(self.batchnorm(deconv2, is_train, name='debn2'))

            # 64->128
            deconv1 = self.new_deconv_layer(debn2, [4, 4, 32, 32], conv1.get_shape().as_list(), stride=2, name="deconv1")
            debn1 = tf.nn.relu(self.batchnorm(deconv1, is_train, name='debn1'))

            # 128->256
            recon = self.new_deconv_layer(debn1, [4, 4, 3, 32], [batch_size, 256, 256, 3], stride=2, name="recon")

        return bn1, bn2, bn3, bn4, bn5, debn5, debn4, debn3, debn2, debn1, recon, tf.nn.tanh(recon)
    ...
    def build_adversarial_random(self, images, is_train, reuse=None):
        with tf.variable_scope('DIS', reuse=reuse):
            conv1_add1 = self.new_conv_layer(images, [4, 4, 3, 32], stride=2, name="conv1_add1")
            bn1_add1 = self.leaky_relu(self.batchnorm(conv1_add1, is_train, name='bn1_add1'))

            conv1_add2 = self.new_conv_layer(bn1_add1, [4, 4, 32, 32], stride=2, name="conv1_add2")
            bn1_add2 = self.leaky_relu(self.batchnorm(conv1_add2, is_train, name='bn1_add2'))

            conv1 = self.new_conv_layer(bn1_add2, [4,4,32,64], stride=2, name="conv1" )
            bn1 = self.leaky_relu(self.batchnorm(conv1, is_train, name='bn1'))
            conv2 = self.new_conv_layer(bn1, [4,4,64,128], stride=2, name="conv2")
            bn2 = self.leaky_relu(self.batchnorm(conv2, is_train, name='bn2'))
            conv3 = self.new_conv_layer(bn2, [4,4,128,256], stride=2, name="conv3")
