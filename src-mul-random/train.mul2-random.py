...
model_path = '../models/Mul-random/'
result_path= '../results/Mul-random/'
...
# Frizy changed : input image 128-->256
...
# Frizy changed : input image hiding_size 64-->resultDim 256
images_hiding = tf.placeholder( tf.float32, [batch_size, resultDim, resultDim, 3], name='images_hiding')

model = Model()

bn1, bn2, bn3, bn4, bn5, debn5, debn4, debn3, debn2, debn1, reconstruction_ori, reconstruction = model.build_reconstruction_random(images_tf, is_train)

adversarial_pos = model.build_adversarial_random(images_hiding, is_train)
adversarial_neg = model.build_adversarial_random(reconstruction, is_train, reuse=True)
...
loss_recon_reduce = tf.square( images_hiding - reconstruction )
loss_recon = tf.reduce_mean(tf.sqrt( 1e-5 + tf.reduce_sum(loss_recon_reduce, [1,2,3])))  # Loss
...
for epoch in range(n_epochs):
    ...
    for start,end in zip(
        ...
        images_ori_all = map(lambda left, right: load_image_mul_random(left, right, height=resultDim, width=resultDim),
        ...
        images_allAndcrops = map(lambda x: crop_random_all_damage(x, x=fill_one_side, y=fill_one_side), images_ori_all)
        ...
        if iters % 500 == 0:
            ...
            test_images_ori_all = map(lambda left, right: load_image_mul_random(left, right, height=resultDim, width=resultDim),
                test_image_paths_left, test_image_paths_right)

            # Frizy
            test_images_allAndcrop = map(
                lambda x: crop_random_all_damage(x, width=hiding_size, height=hiding_size, x=fill_one_side, y=fill_one_side),
                test_images_ori_all)
            test_images_all, test_crops, xs, ys = zip(*test_images_allAndcrop)

            reconstruction_vals, adv_pos_val, adv_neg_val, recon_ori_vals, bn1_val,bn2_val,bn3_val,bn4_val,bn5_val,debn5_val,debn4_val, debn3_val, debn2_val, debn1_val, loss_G_val, loss_D_val = sess.run(
                    [reconstruction, adversarial_pos, adversarial_neg, reconstruction_ori, bn1,bn2,bn3,bn4,bn5,debn5,debn4, debn3, debn2, debn1, loss_G, loss_D],
                    feed_dict={
            ...
            if iters % 500 == 0:
                ii = 0
                for rec_val, img in zip(reconstruction_vals, test_images_all):
                    rec_hid = (255. * (rec_val+1)/2.).astype(int)
                    rec_con = (255. * (img[:,:,0:3]+1)/2.).astype(int)

                    cv2.imwrite( os.path.join(result_path, 'img_'   + str(ii) + '.'+str(int(iters/100)) + '_damage.png'), rec_con)
                    cv2.imwrite( os.path.join(result_path, 'img_' + str(ii) + '.' + str(int(iters / 100))+'_reconstr.png'),rec_hid)
                    ii += 1
        ...
        _, loss_G_val, adv_pos_val, adv_neg_val, loss_recon_val, loss_adv_val, reconstruction_vals, recon_ori_vals, bn1_val,bn2_val,bn3_val,bn4_val,bn5_val,debn5_val,debn4_val, debn3_val, debn2_val, debn1_val = sess.run(
                [train_op_G, loss_G, adversarial_pos, adversarial_neg, loss_recon, loss_adv, reconstruction, reconstruction_ori, bn1,bn2,bn3,bn4,bn5,debn5,debn4, debn3, debn2, debn1],
                feed_dict={
